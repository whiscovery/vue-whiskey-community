...
<div class="row align-items-center">
  ...
  <div class="wrapper" v-for="comment,i in comments" :key="i">
      <div class="speech-bubble md-3 mt-3">
        ...
        <svg class="svg-icon ms-1" width="16" viewBox="0 0 20 20" @click="delComment(comment._id)">
...
export default {
  ...
  async created () {
    const url = baseurl + '/comment/' + this.$route.params.id
    axios.get(url)
      .then((res) => {
    ...
    delComment (value) {
      ...
      if (confirm('코멘트를 삭제하시겠습니까?')) {
        ...
        .then((res) => {
          if (res.status === 200) {
            alert('삭제되었습니다.')
            this.$router.push({ name: 'Detail', params: { id: this.paramsid } }).catch(() => {})
          }
        })
        .catch((err) => {
          console.log(err)
